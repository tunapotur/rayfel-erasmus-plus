...
import { Content, contentType } from "@/lib/types/DataTypes";
...
function CardBadge({ card_NewsType }: { card_NewsType: contentType }) {
  const t = useTranslations("NewsType");

  const badgeColors: Record<contentType, string> = {
    ...
    <div className="bg-background dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-900 shadow-sm p-6 flex flex-col gap-2 hover:shadow-md transition-shadow">
      ...
      {card.headline_image && (
        <div className="relative w-full h-48 rounded-lg overflow-hidden">
          ...
          {["news", "mobility", "dissemination"].includes(card.type) && (
